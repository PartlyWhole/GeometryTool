...
export const WALKTHROUGHS: Walkthrough[] = [
  ...
  },
  {
    ...
    steps: [
      ...
      { text: "Move one of them off the line and the three are no longer collinear. That is why “X, Y and Z are collinear” is only sometimes true — it depends on the points.", figure: "notCollinear", marks: [given(pt("A")), prove(pt("B")), given(pt("C"))], assert: [{ statement: { k: "collinear", pts: ["A", "B", "C"] }, holds: false }] },
    ],
  },
  ...
  // ---------------------------------------------------------------- §6
  {
    ...
    steps: [
      ...
      { text: "Read backwards it is a subtraction, which is how most questions disguise it: know the whole and one part, and the other part follows.", figure: "collinear", marks: [given(seg("A", "C")), given(seg("A", "B")), prove(seg("B", "C"))], show: { k: "eq", l: add(len("A", "C"), mul(num(-1), len("A", "B"))), r: len("B", "C") } },
    ],
  },
  {
    conceptId: "congruent-segments",
    steps: [
      { text: "Congruence is shown with tick marks, and the marks are the only evidence you have. Four segments here carry them.", figure: "markedPair", marks: [given(seg("A", "D")), given(seg("B", "C")), given(seg("E", "B")), given(seg("D", "F"))] },
  ...
  },
  {
    ...
    steps: [
      ...
      { text: "So each half is exactly half the whole, which is usually what a question actually wants.", marks: [given(seg("A", "M")), given(seg("M", "B")), { ...prove(seg("A", "B")), lane: 2 }], show: { k: "eq", l: len("A", "M"), r: { k: "div", n: { k: "len", seg: seg("A", "B") }, d: { k: "num", v: 2 } } } },
  ...
  },
  {
    ...
    steps: [
      ...
      { text: "MQ and MR run straight through M, so together they bisect AB — and they cross at no particular angle. A bisector does not have to be perpendicular.", marks: [given(seg("M", "Q")), given(seg("M", "R")), shared(seg("A", "B"))], assert: [{ statement: { k: "angleClass", ang: ang("AMQ"), cls: "right" }, holds: false }] },
      ...
      { text: "Tilt it to 90° and you have a perpendicular bisector — a special case. The ticks are still there; the square is what is new. Every perpendicular bisector is a bisector, but not the reverse.", figure: "perpBisector", marks: [given(ang("APQ")), given(seg("A", "P")), given(seg("P", "B"))] },
    ],
  },
  {
    ...
    steps: [
      ...
      { text: "Every perpendicular bisector is a bisector, but not every bisector is perpendicular — the arrow only points one way. What makes it worth naming is that it hands you both facts in a proof: the halves are equal, and the angle is 90°." },
    ],
  },
  {
    ...
    steps: [
      ...
      { text: "The other angle at P is 90° too — but the figure only marks one, and only what is marked may be used. To use it you have to derive it: ∠APB is a straight angle, so ∠QPB is 180° − 90°.", marks: [prove(ang("QPB"))], assert: [{ statement: { k: "angleClass", ang: ang("QPB"), cls: "right" }, holds: true }] },
    ],
  },
  ...
  // ---------------------------------------------------------------- §8
  {
    ...
    steps: [
      ...
      { text: "Strictly less than 90°. The angle beside it measures exactly 90°, and that one is right, not acute — the boundary is not included.", marks: [given(ang("APB")), prove(ang("CQD"))] },
    ],
  },
  {
    conceptId: "right",
  ...
  },
  {
    ...
    steps: [
      ...
      { text: "Strictly less than 180°. The flat one on the right also exceeds 90°, and it is straight, not obtuse — that is the usual trap.", marks: [given(ang("ERF")), prove(ang("GSH"))] },
      { text: "So the name covers a range, not a single value: anything strictly between the square and the flat line.", marks: [given(ang("ERF"))] },
    ],
  },
  {
    conceptId: "straight",
    steps: [
      ...
      { text: "It is easiest to miss when another line runs through the vertex. Here ∠EXF is straight, and questions like “one angle is ⅝ of another” lean on your not noticing.", figure: "straightInDisguise", marks: [given(ang("EXF"))], assert: [{ statement: { k: "angleClass", ang: ang("EXF"), cls: "straight" }, holds: true }] },
    ],
  },
  {
    conceptId: "congruent-angles",
    steps: [
      { text: "Congruent angles are shown with matching arcs.", figure: "congruentAnglesPair" },
      { text: "∠AVB and ∠CWD each carry one arc, so they are congruent — even though they point different ways and sit in different corners.", marks: [given(ang("AVB")), given(ang("CWD"))], assert: [{ statement: { k: "cong", l: ang("AVB"), r: ang("CWD") }, holds: true }] },
      { text: "One arc is drawn small and the other large, and they still mark the same congruence. The size of an arc means nothing; the number of arcs is what matches.", marks: [given(ang("AVB")), given(ang("CWD"))] },
  ...
  },
  {
    ...
    steps: [
      ...
      { text: "It is the special case of an interior ray where the parts come out equal — so each half is exactly half the whole, and you can write that as an equation straight away.", show: { k: "eq", l: mul(num(2), meas("AVD")), r: meas("AVC") }, marks: [prove(ang("AVC"))] },
    ],
  },
  {
    conceptId: "adjacent",
    steps: [
      ...
      { text: "∠1 and ∠3 are not adjacent. They share the vertex but no side — they face each other instead.", marks: [shared(ang("1")), shared(ang("3"))], assert: [{ statement: { k: "adjacent", a: ang("1"), b: ang("3") }, holds: false }, { statement: { k: "vertical", a: ang("1"), b: ang("3") }, holds: true }] },
  ...
  },
  {
    ...
    steps: [
      ...
      { text: "What makes them vertical is the rays: each side of ∠1 is the opposite ray of a side of ∠3.", marks: [given(ang("1")), given(ang("3")), shared(seg("A", "B")), shared(seg("C", "D"))] },
  ...
  },
  {
    ...
    steps: [
      ...
      { text: "But supplementary angles need not touch at all. Two angles in different diagrams, on different pages, are supplementary if their measures add to 180°.", figure: "twoSupplementsPlain", marks: [given(ang("1")), given(ang("4"))], assert: [{ statement: { k: "supp", a: ang("1"), b: ang("4") }, holds: true }, { statement: { k: "linearPair", a: ang("1"), b: ang("4") }, holds: false }] },
  ...
  },
  {
    ...
    steps: [
      ...
      { text: "And as with supplements, neither angle has to be drawn near the other. These two are 35° and 55° in different corners, and they are still complementary.", figure: "complementsApart", marks: [given(ang("AVB")), given(ang("CWD"))], assert: [{ statement: { k: "comp", a: ang("AVB"), b: ang("CWD") }, holds: true }], show: { k: "eq", l: { k: "add", ts: [{ k: "meas", ang: ang("AVD") }, { k: "meas", ang: ang("DVC") }] }, r: { k: "num", v: 90 } } },
    ],
  },
  {
  ...
  // ---------------------------------------------------------------- §4
  {
    ...
    steps: [
      ...
      { text: "∠XVY belongs to both of the larger angles — to ∠WVY on one side and to ∠XVZ on the other. Writing m∠XVY = m∠XVY is what lets you add it to both sides of something else.", marks: [prove(ang("WVY")), prove(ang("XVZ")), shared(ang("XVY"))], show: { k: "eq", l: meas("XVY"), r: meas("XVY") } },
  ...
  },
  {
    ...
    steps: [
      ...
      { text: "On a figure it is the same move. The ticks here give AD ≅ BC.", figure: "markedPair", marks: [given(seg("A", "D")), given(seg("B", "C"))], show: { k: "cong", l: seg("A", "D"), r: seg("B", "C") }, assert: [{ statement: { k: "cong", l: seg("A", "D"), r: seg("B", "C") }, holds: true }] },
  ...
  },
  {
    ...
    steps: [
      ...
      { text: "Add it, and the two larger angles come out equal — which is the whole of the proof.", marks: [given(ang("WVX")), given(ang("YVZ")), shared(ang("XVY")), prove(ang("WVY")), prove(ang("XVZ"))], show: { k: "eq", l: meas("WVY"), r: meas("XVZ") }, assert: [{ statement: { k: "cong", l: ang("WVY"), r: ang("XVZ") }, holds: true }] },
    ],
  },
  {
  ...
  },
  {
    ...
    steps: [
      ...
      { text: "On a figure: M is the midpoint of AB, so AM is half of AB.", figure: "midpoint", marks: [given(seg("A", "M")), { ...prove(seg("A", "B")), lane: 2 }], assert: [{ statement: { k: "midpoint", p: "M", seg: seg("A", "B") }, holds: true }] },
      { text: "Multiply both sides by 2 and you have the form a proof actually uses: twice the half is the whole.", marks: [given(seg("A", "M")), given(seg("M", "B")), { ...prove(seg("A", "B")), lane: 2 }], show: { k: "eq", l: mul(num(2), len("A", "M")), r: len("A", "B") } },
  ...
  },
  {
    ...
    steps: [
      { text: "Dividing both sides by the same nonzero number keeps them equal.", show: { k: "eq", l: mul(num(2), vr("x")), r: num(76) } },
      { text: "Divide each side by 2.", show: { k: "eq", l: vr("x"), r: num(38) } },
      ...
      { text: "Divide both sides by 2 to get a half on its own — the same divisor as the algebra, and for the same reason: it is what both sides carry.", marks: [given(ang("DVC")), shared(ang("AVC")), prove(ang("AVD"))], show: { k: "eq", l: meas("AVD"), r: div(meas("AVC"), num(2)) } },
  ...
  },
  {
    ...
    steps: [
      ...
      { text: "Read that on a figure. Suppose this one comes with AB = 5 and AB + BC = 12.", figure: "collinear", marks: [given(seg("A", "B")), given(seg("B", "C"))] },
      { text: "AB does appear in both, but it is not a whole side of the second equation — so there is no hinge to turn. The 5 simply stands in for AB, and BC = 7 falls out.", marks: [given(seg("A", "B")), prove(seg("B", "C"))], show: { k: "eq", l: len("B", "C"), r: num(7) } },
  ...
  },
  {
    ...
    steps: [
      ...
      { text: "The picture behind it: AC is one segment made of two unequal parts, AB and BC.", figure: "twoUnequalParts", marks: [given(seg("A", "B")), given(seg("B", "C")), { ...shared(seg("A", "C")), lane: 2 }] },
      { text: "Take three copies of the whole and you have 3·AC. Take three copies of each part instead and you have 3·AB + 3·BC. The same length, counted two ways — which is exactly what the parentheses do.", marks: [given(seg("A", "B")), given(seg("B", "C")), { ...prove(seg("A", "C")), lane: 2 }], show: { k: "eq", l: mul(num(3), add(len("A", "B"), len("B", "C"))), r: add(mul(num(3), len("A", "B")), mul(num(3), len("B", "C"))) } },
  ...
  },
  {
    ...
    steps: [
      ...
      { text: "∠1 and ∠2 form a linear pair, so by the Linear Pair Theorem they total 180°.", marks: [given(ang("1")), shared(ang("2"))], show: { k: "eq", l: add(meas("1"), meas("2")), r: num(180) }, assert: [{ statement: { k: "linearPair", a: ang("1"), b: ang("2") }, holds: true }] },
      { text: "∠2 and ∠3 form a linear pair too, so the same theorem gives their total.", marks: [shared(ang("2")), given(ang("3"))], show: { k: "eq", l: add(meas("2"), meas("3")), r: num(180) } },
      { text: "Two quantities equal to the same thing are equal to each other.", marks: [given(ang("1")), shared(ang("2")), given(ang("3"))], show: { k: "eq", l: add(meas("1"), meas("2")), r: add(meas("2"), meas("3")) } },
  ...
  },
  {
    ...
    steps: [
      ...
      { text: "The arcs mark ∠3 ≅ ∠4, so the parts being subtracted are equal — and substituting one for the other gives two sums that share a term.", marks: [shared(ang("3")), shared(ang("4"))], show: { k: "eq", l: add(meas("1"), meas("3")), r: add(meas("2"), meas("3")) }, assert: [{ statement: { k: "cong", l: ang("3"), r: ang("4") }, holds: true }, { statement: { k: "supp", a: ang("1"), b: ang("3") }, holds: true }] },
      { text: "Subtract that shared term and what is left must be equal: ∠1 ≅ ∠2. Note that these two never touch — being supplements of congruent angles is enough.", marks: [shared(ang("3")), prove(ang("1")), prove(ang("2"))], show: { k: "eq", l: meas("1"), r: meas("2") }, assert: [{ statement: { k: "cong", l: ang("1"), r: ang("2") }, holds: true }, { statement: { k: "adjacent", a: ang("1"), b: ang("2") }, holds: false }] },
  ...
  },
  {
    ...
    steps: [
      ...
      { text: "The square marks ∠1 as 90°.", marks: [given(ang("1"))], show: { k: "eq", l: meas("1"), r: num(90) } },
      { text: "The other square marks ∠2 as 90°, in a different place entirely.", marks: [given(ang("1")), given(ang("2"))], show: { k: "eq", l: meas("2"), r: num(90) } },
      { text: "Two quantities each equal to 90 are equal to each other, so ∠1 ≅ ∠2.", marks: [prove(ang("1")), prove(ang("2"))], show: { k: "eq", l: meas("1"), r: meas("2") }, assert: [{ statement: { k: "angleClass", ang: ang("1"), cls: "right" }, holds: true }, { statement: { k: "angleClass", ang: ang("2"), cls: "right" }, holds: true }, { statement: { k: "cong", l: ang("1"), r: ang("2") }, holds: true }] },
  ...
  // ---------------------------------------------------------------- §2, §3
  {
    ...
    steps: [
      ...
      { text: "Three cases agree, so you conjecture that every linear pair totals 180°. That is inductive reasoning, and this conjecture does happen to be true.", figure: "linearPairCaseC", marks: [given(ang("ABD")), given(ang("DBC"))] },
      { text: "Now watch one go wrong. These two near-vertical segments look the same length, and the ticks agree that they are.", figure: "markedPair", marks: [given(seg("A", "D")), given(seg("B", "C"))], assert: [{ statement: { k: "cong", l: seg("A", "D"), r: seg("B", "C") }, holds: true }] },
      { text: "A third looks the same again, so the pattern is two for two and the conjecture forms: all three are equal.", marks: [given(seg("A", "D")), given(seg("B", "C")), shared(seg("E", "F"))] },
      { text: "Measure it and the pattern breaks. EF is longer than the other two, and it carries no tick — one case is all it takes to end a conjecture.", marks: [prove(seg("E", "F"))], assert: [{ statement: { k: "cong", l: seg("B", "C"), r: seg("E", "F") }, holds: false }] },
      { text: "That is the whole difference. Cases can suggest a rule and can refute one, but they can never establish it — which is why geometry finishes its arguments some other way." },
    ],
  },
  {
    conceptId: "deductive",
